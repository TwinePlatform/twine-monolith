...
export type CommunityBusiness = Organisation &
  Readonly<{
    region: RegionEnum;
    sector: SectorEnum;
    logoUrl: string;
...
export type VisitActivity = Readonly<
  CommonTimestamps & {
    id: Int;
    name: string;
    category: string;
...
export type VisitEvent = Readonly<
  CommonTimestamps & {
    id: Int;
    userId: Int;
    visitActivityId: Int;
...
export type LinkedVisitEvent = VisitEvent &
  Readonly<{
...
export type LinkedFeedback = Feedback &
  Readonly<{
    organisationId: Int;
  }>;

export type SingleUseToken = Readonly<
  Omit<CommonTimestamps, "modifiedAt"> & {
    id: Int;
    userId: Int;
    token: string;
    expiresAt: string;
...
export type VolunteerActivity = Readonly<
  CommonTimestamps & {
    id: Int;
    name: string;
  }
...
export type VolunteerProject = Readonly<
  CommonTimestamps & {
    id: Int;
    name: string;
    organisationId: Int;
...
export type VolunteerLog = Readonly<
  CommonTimestamps & {
    id: Int;
    userId: Int;
    createdBy?: Int;
...
export type CommunityBusinessCollection = Collection<CommunityBusiness> & {
  ...
  addVisitActivity: (
    k: Knex,
    v: Partial<VisitActivity>,
    c: Partial<CommunityBusiness>
  ) => Promise<Maybe<VisitActivity>>;
  updateVisitActivity: (
    k: Knex,
    a: Partial<VisitActivity>
  ) => Promise<Maybe<VisitActivity>>;
  ...
  addVisitLog: (
    ...
    v: VisitActivity,
    u: Partial<User>,
    a: "sign_in_with_name" | "qr_code"
  ) => Promise<VisitEvent>;
  // TODO [getVisitLogsWithUsers]:
  // this is still wrong, we return "category", which is missing
  getVisitLogsWithUsers: (
    k: Knex,
    c: CommunityBusiness,
    q?: ModelQuery<LinkedVisitEvent & User>
  ) => Promise<
    Partial<LinkedVisitEvent & Pick<User, "birthYear" | "gender">>[]
  >;
  ...
  getTemporary: (
    k: Knex
  ) => Promise<Pick<CommunityBusiness, "id" | "name"> & CommonTimestamps>;
...
export type VolunteerLogCollection = Collection<VolunteerLog> & {
  ...
  projectToColumnNames: (
    a: Partial<Record<keyof VolunteerProject, any>>
  ) => Dictionary<any>;
  fromUser: (
    k: Knex,
    u: User,
    q?: ModelQuery<VolunteerLog>
  ...
  getProjects: (
    k: Knex,
    c: CommunityBusiness,
    q?: ModelQuery<VolunteerProject>
  ) => Promise<VolunteerProject[]>;
  addProject: (
    k: Knex,
    c: CommunityBusiness,
    name: string
  ...
  updateProject: (
    k: Knex,
    p: VolunteerProject,
    c: Partial<VolunteerProject>
  ) => Promise<VolunteerProject[]>;
  deleteProject: (k: Knex, p: VolunteerProject) => Promise<Int>;
  restoreProject: (k: Knex, p: VolunteerProject) => Promise<Int>;
  syncLogs: (
    k: Knex,
    c: CommunityBusiness,
    u: User,
    ls: Partial<VolunteerLog>[]
  ) => Promise<{ synced: Int; ignored: Int }>;
};
...
export type RolesCollection = {
  ...
  fromUser: (
    k: Knex,
    a: User
  ) => Promise<{ organisationId: Int; role: RoleEnum }[]>;
