...
export const Utils = {
  ...
  whereNotBetween: <T, U>(o: Dictionary<[DateLike, DateLike]>) =>
    (q: Knex.QueryBuilder<T, U>) =>
      Object.keys(o)
        .reduce((queryChain, whereQuery, i) => {
          if (o[whereQuery].length !== 2) {
            ...
            return queryChain.whereNot(whereQuery, '<', o[whereQuery][1]);
          } else if (typeof o[whereQuery][1] === 'undefined') {
            return queryChain.whereNot(whereQuery, '>', o[whereQuery][0]);
          } else {
            return queryChain.whereNotBetween(whereQuery, o[whereQuery]);
          }
        }, q),
};

export function applyQueryModifiers<T, U, V>(p: Knex.QueryBuilder<T, U>, opts: ModelQuery<V>): Knex.QueryBuilder<T, U>;
export function applyQueryModifiers<T, U, V>(p: Knex.QueryBuilder<T, U>, opts: ModelQueryPartial<V>): Knex.QueryBuilder<T, U>;
export function applyQueryModifiers<T, U, V>(p: Knex.QueryBuilder<T, U>, opts: any): Knex.QueryBuilder<T, U> {
  ...
  const query = modifiers
    .reduce((acc, k: keyof ModelQuery<U>) => {
      ...
      } else if (k === 'whereNotBetween' && !isNil(opts.whereNotBetween)) {
        return compose(Utils.whereNotBetween(opts.whereNotBetween), acc);
