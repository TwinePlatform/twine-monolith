...
const uniformLogs = (user: User) => (
  log: SyncLogPayloadItem
): Partial<VolunteerLog> => ({
  ...log,
...
const routes: [
...
] = [
  {
    ...
    options: {
      ...
      pre: [{ method: getCommunityBusiness, assign: "communityBusiness" }],
    },
    handler: async (request, h) => {
      ...
      const logs = await VolunteerLogs.fromCommunityBusiness(
        knex,
        communityBusiness,
        query
      );
...

  {
    ...
    handler: async (request, h) => {
      ...
      const log = await VolunteerLogs.getOne(knex, {
        where: {
          id: Number(logId),
...

  {
    ...
    handler: async (request, h) => {
      ...
      const log = await VolunteerLogs.destroy(knex, {
        id: Number(logId),
        organisationId: communityBusiness.id,
      });
...

  {
    ...
    handler: async (request, h) => {
      ...
      try {
        ...
        if (error.code === "23502") {
          // Violation of null constraint implies invalid activity
...

  {
    ...
    options: {
      description:
        "Synchronise volunteer logs for own use at community business",
      auth: {
        strategy: "standard",
        access: {
          scope: [
    ...
    handler: async (request, h) => {
      ...
      try {
        const result = await VolunteerLogs.syncLogs(
          knex,
          communityBusiness,
          user,
          payload
...

  {
    ...
    options: {
      description:
        "Retreive summary of volunteer statistics from own organisation",
      auth: {
        strategy: "standard",
        access: {
          scope: [
    ...
    handler: async (request, h) => {
      ...
      const logs = await VolunteerLogs.get(knex, {
        fields: ["duration"],
      ...
      const total = logs.reduce(
        (acc, log) => Duration.sum(acc, log.duration),
        Duration.fromSeconds(0)
      );

      const volunteers = await Volunteers.fromCommunityBusiness(
        knex,
        communityBusiness
      );
