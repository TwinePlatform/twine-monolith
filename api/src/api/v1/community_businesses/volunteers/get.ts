...
const routes: [
...
] = [
  {
    ...
    handler: async (request, h) => {
      ...
      const count =
        has("limit", modelQuery) || has("offset", modelQuery)
      ...
      return {
        result: await Promise.all(
          volunteers.map(Serialisers.volunteers.noSecrets)
        ),
        meta: { total: count },
      };
    },
  },
  {
    ...
    options: {
      description:
        "Retreive list of push notification token of all volunteers from a specific project",
      auth: {
        strategy: "standard",
        access: {
          scope: ["user_details-child:read", "user_details-sibling:read"],
    ...
    handler: async (request, h) => {
      ...
      const volunteers = await Volunteers.fromProjectWithToken(
        knex,
        communityBusiness,
        projectId
      );
