...
const main = async (config: Config, client: Knex) => {
  ...
  const rows = res.map((row) => {
    ...
    const sessionLength = moment(processedRow.sessionEndedAt).subtract(processedRow.sessionStartedAt);
    const headers = row.headers.map(processHeaders);
    const app = mapHostToApp(pathOr(undefined, ['url', 'host'], headers[0]));

    if (app !== 'dashboard-app') {
      return null;
    }
    ...
    return {
      month: moment(row.sessionStartedAt).format('MMM YYYY'),
      user: row.userId,
      sessionLength,
      app,
      pages,
  ...
  const months = uniq(res.map((row) => row.sessionStartedAt).sort().map((date) => moment(date).format('MMM YYYY')));
