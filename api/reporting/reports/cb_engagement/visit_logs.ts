...
import { CommunityBusiness, CommunityBusinesses } from '../../../src/models';
...
export default async () => {
  ...
  const data = await Promise.all(cbs.map(async (cb) => {
    ...
    const months = await dates.reduce(async (_acc, date) => {
      ...
      acc[date.format('MMM YYYY')] = await cbLogsInMonthVis(client, date, cb);
      return acc;
...
const cbLogsInMonthVis = async (client: Knex, date: moment.Moment, cb: CommunityBusiness) => {
